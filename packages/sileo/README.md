...
Set the viewport to any of Sileo's six original positions with the
...
Supported positions are `top-left`, `top-center`, `top-right`, `bottom-left`,
`bottom-center`, and `bottom-right`; the default is `top-center`. The toast pill
aligns with the selected viewport edge while keeping the original Sileo
animations.
